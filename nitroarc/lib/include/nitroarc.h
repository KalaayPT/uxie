...
#define NITROARC_VERSION_MINOR 0
#define NITROARC_VERSION_PATCH 1
...
/*
 * Get the exact size required to store the name of an archive member with a
 * given integer index, including the zero-terminator.
 *
 * If `i` is some value greater than the number of members present in the
 * archive, then a non-zero error code will be returned. If the input archive
 * does not record file-names, then `*out_size` will be set to 1.
 */
