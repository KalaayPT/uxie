...
pub fn parse_defines(source: &str) -> Vec<CDefine> {
    ...
    let pattern =
        Regex::new(r"#define\s+([A-Za-z_][A-Za-z0-9_]*)\s+(.+?)(?:\s*(?://|/\*).*)?$").unwrap();
...
#define SOME_VALUE 0x10
    ...
    fn test_nested_references() {
        let defines = vec![
            ...
            CDefine {
                name: "TRAINER_DOUBLES".into(),
                value: "(DOUBLES | TRAINER)".into(),
                resolved: None,
            },
            CDefine {
                name: "LINK".into(),
            ...
            CDefine {
                name: "LINK_DOUBLES".into(),
                value: "(LINK | TRAINER_DOUBLES)".into(),
                resolved: None,
            },
        ];
