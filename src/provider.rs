...
use crate::map_header::{MapHeader, read_map_headers_from_arm9, read_map_header_from_bytes, MAP_HEADER_SIZE};
...
impl Arm9Provider {
    pub fn new(arm9_path: impl AsRef<Path>, header_table_offset: u64, header_count: usize, game_family: GameFamily) -> Self {
        Self {
    ...
    fn read_all_headers(&self) -> io::Result<Vec<MapHeader>> {
        ...
        read_map_headers_from_arm9(&mut file, self.header_table_offset, self.header_count, self.game_family)
    }
}
    ...
    fn get_map_header(&self, id: u16) -> io::Result<MapHeader> {
        if id as usize >= self.header_count {
            return Err(io::Error::new(
                ...
                format!("Header ID {} out of range (max {})", id, self.header_count - 1),
            ));
        }
